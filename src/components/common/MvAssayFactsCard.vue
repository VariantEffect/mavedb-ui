  ...
  <div>
    ...
    <div v-if="scoreSet" class="mb-2.5 pb-2.5 border-b border-border">
      <div class="text-base font-semibold text-text-primary leading-[1.3] mb-0.5">
        <router-link class="text-link" :to="{name: 'scoreSet', params: {urn: scoreSet.urn}, query: variantQuery}">
    ...
    <template v-if="hasOddsPath">
      <div class="flex flex-col">
        <MvDetailRow fallback="Not provided" label="Normal">
          <template v-if="normalOddsPath != null">
            <span class="font-mono font-bold inline-block min-w-[7ch] text-right">{{ normalOddsPath }}</span>
            <MvEvidenceTag v-if="normalEvidenceCode" class="ml-2" :code="normalEvidenceCode" />
          </template>
        </MvDetailRow>
        <MvDetailRow fallback="Not provided" label="Abnormal">
          <template v-if="abnormalOddsPath != null">
            <span class="font-mono font-bold inline-block min-w-[7ch] text-right">{{ abnormalOddsPath }}</span>
