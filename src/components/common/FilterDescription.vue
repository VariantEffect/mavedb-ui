...
export default {
  ...
  computed: {
    clauses: function() {
      return this.filter.and || [this.filter] || [null]
    },
    text: function() {
      const clauses = this.clauses.filter((c) => c != null)
      if (clauses.length == 0) {
        return ''
      } else if (clauses.length == 1) {
        return this.clauseText(clauses[0]) || ''
      } else {
        return (this.clauseText(clauses[0]) || '') + ` + ${clauses.length - 1} more`
      }
    }
  },
  ...
  methods: {
    ...
    expressionDataType: function(expression) {
      if (expression.path) {
        const filterProperty = this.filterProperties.find((fp) => fp.path == expression.path)
    ...
    expressionText: function(expression, dataType = null) {
      if (expression.path) {
        ...
        const filterProperty = this.filterProperties.find((fp) => fp.text == expression.text)
