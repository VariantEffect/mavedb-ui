...
export default vuexOidcCreateStoreModule(
  {
    ...
    responseType: 'id_token token', // Or 'code' for PKCE
    scope: 'openid',
    automaticSilentRenew: false,
    automaticSilentSignin: false,

    // Tell ORCID to re-authenticate the user during the login process. Otherwise, since ORCID does not provide a logout
    // URL that we could use to fully log the user out, the user stays logged into ORCID even though we have ended the
    // session on the client side. The next time the user presses "Sign in," ORCID will log them in with the same
    // credentials before, making it impossible to change users without clearing cookies.
    prompt: 'login',

    // URIs for redirecting the browser back to MaveDB
    redirectUri: `${appUrl}oidc-callback`,
    silentRedirectUri: `${appUrl}silent-renew-oidc.html`,
    post_logout_redirect_uri: `${appUrl}oidc-callback`
  },
...
// silent-renew-oidc.html should load the following script:
