    ...
    downloadFile: async function(download_type) {
      ...
      try {
        if (this.item && download_type == "counts") {
          response = await axios.get(`${config.apiBaseUrl}/score-sets/${this.item.urn}/counts?download=true`)
        } else if (this.item && download_type == "scores") {
          response = await axios.get(`${config.apiBaseUrl}/score-sets/${this.item.urn}/scores?download=true`)
        }
      } catch (e) {
        response = e.response || { status: 500 }
      }
