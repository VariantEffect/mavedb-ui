...
export default function makeBarLineChart(): BarLineChart {
    ...
    let datumSorter: ((
        datumA: BarLineChartDatum, datumB: BarLineChartDatum,
    ) => number) | null = null;
    ...
    const line = d3.line<BarLineChartDatum>()
        .x(d => xScale(d.x))
    ...
    const renderTooltips = () => {
        _hoverTooltip = d3.select(document.body)
            .append('div')
            .style('display', 'none')
    ...
    const hideHighlight = (d: BarLineChartDatum, nodeName: string) => {
        if (svg) {
            svg.select(`g.${nodeName}s`).selectAll(`${nodeName}.node-${d.index}`)
    ...
    const chart: BarLineChart = {
        ...
        refresh: () => {
            if (_container && svg) {
                ...
                yScale1.domain([
                    0,
                    d3.max(data, d => aggregateData ? d.aggregateY1 : d.y1) as number
                ]).range([height - margins.bottom, margins.top]);

                yScale2.domain([
                    0,
                    d3.max(data, d => aggregateData ? d.aggregateY2 : d.y2) as number
                ]).range([height - margins.bottom, margins.top]);
                ...
                svg.select('g.rects').selectAll('rect')
                    ...
                    .attr('width', barWidth)
                    .attr('height', d => d ? height - margins.bottom - (aggregateData ? yScale1(d.aggregateY1) : yScale1(d.y1)) : 0)
                    .style('fill', barColor.toString());

                svg.select('g.circles').selectAll('circle').remove();
                svg.select('g.circles').selectAll('circle')
                    .data(data, (d: BarLineChartDatum) => d.index)
                    .enter()
                    .append('circle')
                    .attr('class', d => `node-${d.index}`)
