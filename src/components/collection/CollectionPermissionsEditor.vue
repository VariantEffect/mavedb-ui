...
export default defineComponent({
  ...
  components: {PButton, Column, DataTable, PDialog, PSelect, MvEmailPrompt, InputText, MvFloatField},
  ...
  methods: {
    ...
    changeRole(orcidId: string, newRole: CollectionRole): void {
      ...
      if (userRole) {
        if (userRole.state === 'saved') {
          ...
          userRole.role = newRole
          const pendingUserRoleIndex = this.pendingUserRoles.findIndex(
            (ur) => ur.user.orcidId === userRole.user.orcidId
          )
    ...
    async saveChanges(): Promise<void> {
      ...
      const totalAttempted = this.orcidIdsToRemove.length + this.pendingUserRoles.length + userRoles.length
      const totalFailed =
        failedRemovalUserRoles.length + failedAdditionUserRoles.length + failedRoleChangeUserRoles.length

      if (totalFailed === 0) {
        this.dialogVisible = false
      }

      if (totalAttempted > totalFailed) {
        this.$emit('saved')
      }
    }
  }
